...
export function whisperChaiOrder(order) {
  ...
  const whisperOrder = order.trim();
  if(whisperOrder ==="") return "";
  return whisperOrder.toLowerCase();
}

export function hasSpecialIngredient(order, ingredient) {
  if(typeof order!=='string' || typeof ingredient!=='string')  return false;
  const ORDER = order.trim().toLowerCase();
  const INGRED = ingredient.trim().toLowerCase();
  if(ORDER.includes(INGRED)) return true;
  return false;
}
